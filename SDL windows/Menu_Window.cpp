...
bool loadMedia()
{
    ...
    sound = Mix_LoadWAV( "music/bloop.wav" );
	if( sound == NULL )
	{
		std::cout << "SDL_Mixer Error: " << Mix_GetError() << std::endl;
		success = false;
	...
	else
	{
		...
		else
		{
			...
			SDL_SetRenderDrawColor(render, 0,0,255,255);
  		 	...
  		 	TTF_Font * font = TTF_OpenFont("Averia.ttf", 50);
    		SDL_Color black = {0, 0, 0};
    		SDL_Color white = {255,255,255};
  		 	SDL_SetRenderDrawColor(render,255,255,255,255);

  		 	surface = TTF_RenderText_Solid(font, "Title?", white);
    		texture = SDL_CreateTextureFromSurface(render, surface);

			SDL_Rect title;
    		title.x = (SWIDTH/4)-120;    
    		title.y = (SHEIGHT/4);    
    		title.w = SWIDTH-100;  
    		title.h = 100;   

    		SDL_RenderCopy(render, texture, NULL, &title);


    		surface = TTF_RenderText_Solid(font, "ATRLock", white);
    		texture = SDL_CreateTextureFromSurface(render, surface);
    		...
    		surface = TTF_RenderText_Solid(font, "Animation", white);
    		texture = SDL_CreateTextureFromSurface(render, surface);
